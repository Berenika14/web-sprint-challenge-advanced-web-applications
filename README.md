...
1. Explain what a token is used for.
   ...
   - Token is used as an authentication signature. For example whenever a user logs in an app the system needs to authenticate the user and instead on repeating this process for every request, a token is created that represents the fact that the user is been authenticated and authorized to have access to the data of the web app.

2. What steps can you take in your web apps to keep your data secure?

The api token has to be send with every request , so in that way if the user is not authenticated it will not have authorization to see or update the data.

3. Describe how web servers work.

A web server is a computer that stores thh code for a website.
The computer is connected to the internet and uses the code that is stored to share it in in the all world at all time.When we load the code for our websites, or web apps, on a server like this, we say that the server is hosting our website app.

4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
   Create = Post : A new entry is created
   Read = Get : returns all the entries
   Update = Put : updates an existing entry
   Delete = Delete : deletes existing entry
